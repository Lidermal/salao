...
const U = {
    ...
    fillTemplate(text, vars) {
        let out = text || '';
        Object.keys(vars || {}).forEach(k => { if (k.startsWith('_')) return; out = out.split(`{${k}}`).join(vars[k] ?? ''); });
        return out;
    },
    ...
    getQuinzenaDates(val) {
        ...
        const [y, m, q] = val.split('-'); const lastDay = new Date(y, m, 0).getDate();
        if (q === 'Q1') return { start: `${y}-${m}-01T00:00:00Z`, end: `${y}-${m}-15T23:59:59Z` };
        return { start: `${y}-${m}-16T00:00:00Z`, end: `${y}-${m}-${lastDay}T23:59:59Z` };
    },
    buildExtrato(desp) {
        let extrato = []; let totalIn = 0, totalOut = 0;
        (desp || []).forEach(d => {
            const isIncome = App.inflowCategories.includes(d.category); const valNum = Number(d.amount) || 0;
            const item = { type: isIncome ? 'in' : 'out', desc: d.description, val: valNum, date: new Date(d.date), category: d.category };
            if(isIncome) totalIn += valNum; else totalOut += valNum; extrato.push(item);
        });
        extrato.sort((a, b) => a.date - b.date); let saldoAtual = 0;
        extrato = extrato.map(item => { saldoAtual += item.type === 'in' ? item.val : -item.val; return { ...item, saldo: saldoAtual }; });
        extrato.reverse(); return { extrato, totalIn, totalOut };
    },
    formatDesc: (text) => {
    ...
    render(id, placeholder, optionsHtml, onChangeGlobalName = '', initialValue = '') {
        ...
        <div class="aqc-custom-select" id="wrapper-${id}" onclick="event.stopPropagation()">
            <div class="aqc-select-trigger" onclick="CustomSelect.toggle('${id}')">
                <span id="label-${id}">${placeholder}</span><i class="ph ph-caret-down"></i>
            </div>
            <div class="aqc-select-menu" id="menu-${id}" style="display:none;">
                <div class="aqc-select-search-box"><input type="text" placeholder="Buscar..." onkeyup="CustomSelect.filter('${id}', this.value)"></div>
                <ul class="aqc-select-options" id="options-${id}">${optionsHtml}</ul>
            </div>
            <input type="hidden" id="${id}" value="${initialValue}" required onchange="${onChangeGlobalName ? onChangeGlobalName + '(this.value)' : ''}">
        </div>`;
    },
    toggle(id) {
        const menu = document.getElementById(`menu-${id}`); const wrapper = document.getElementById(`wrapper-${id}`);
        ...
        document.querySelectorAll('.aqc-custom-select').forEach(el => { el.style.zIndex = '1'; if(el.parentElement) el.parentElement.style.zIndex = ''; });
        if (isOpening) {
            menu.style.display = 'flex'; wrapper.style.zIndex = '999999';
            if(wrapper.parentElement) { wrapper.parentElement.style.position = 'relative'; wrapper.parentElement.style.zIndex = '999999'; }
            const rect = wrapper.getBoundingClientRect(); const spaceBelow = window.innerHeight - rect.bottom;
            if (spaceBelow < 280) { menu.style.top = 'auto'; menu.style.bottom = 'calc(100% + 5px)'; menu.style.boxShadow = '0 -10px 30px rgba(0,0,0,0.25)'; }
            else { menu.style.top = 'calc(100% + 5px)'; menu.style.bottom = 'auto'; menu.style.boxShadow = '0 10px 30px rgba(0,0,0,0.25)'; }
            const input = menu.querySelector('input'); if(input) { input.value = ''; CustomSelect.filter(id, ''); input.focus(); }
        }
    },
    filter(id, term) {
        term = term.toLowerCase(); const opts = document.getElementById(`options-${id}`).querySelectorAll('li');
        opts.forEach(opt => { if (opt.classList.contains('optgroup-label')) return; const text = opt.innerText.toLowerCase(); opt.style.display = text.includes(term) ? 'block' : 'none'; });
    },
    select(id, val, text) {
        document.getElementById(`label-${id}`).innerText = text; const input = document.getElementById(id);
        if(val.includes('%7B')) val = decodeURIComponent(val); input.value = val; input.dispatchEvent(new Event('change'));
        document.getElementById(`menu-${id}`).style.display = 'none';
        const wrapper = document.getElementById(`wrapper-${id}`); wrapper.style.zIndex = '1'; if(wrapper.parentElement) wrapper.parentElement.style.zIndex = '';
    },
    closeAll() {
        document.querySelectorAll('.aqc-select-menu').forEach(el => el.style.display = 'none');
        document.querySelectorAll('.aqc-custom-select').forEach(el => { el.style.zIndex = '1'; if(el.parentElement) el.parentElement.style.zIndex = ''; });
...
const UI = {
    toast(msg, type='success') {
        const cont = document.getElementById('toast-container'); const t = document.createElement('div'); t.className = `toast ${type}`;
    ...
    confirm(msg, onConfirm) {
        document.getElementById('confirm-msg').textContent = msg; const modal = document.getElementById('custom-confirm'); modal.classList.remove('hidden');
...
const Render = {
    ...
    observacoes(id, name) {
        ...
        if (name) { window.currentObsClientName = name; document.getElementById('observacao-title').textContent = `Observações de: ${name}`; }
        else if (window.currentObsClientName) { document.getElementById('observacao-title').textContent = `Observações de: ${window.currentObsClientName}`; }
        Nav.showView('observacoes'); Actions.loadObservacoes(id);
    },
    ...
    async cobrancas(tab = 'pendentes') {
        const tabPendentes = document.getElementById('tab-pendentes-tour'); const tabPagos = document.getElementById('tab-pagos');
        if (tabPendentes) { tabPendentes.classList.toggle('active-tab', tab === 'pendentes'); tabPendentes.style.background = tab === 'pendentes' ? '#f0f0f0' : 'transparent'; tabPendentes.style.border = tab === 'pendentes' ? 'none' : '1px solid transparent'; }
        if (tabPagos) { tabPagos.classList.toggle('active-tab', tab === 'pagos'); tabPagos.style.background = tab === 'pagos' ? '#f0f0f0' : 'transparent'; tabPagos.style.border = tab === 'pagos' ? 'none' : '1px solid transparent'; }
    ...
    async comandas(targetStatus = null, targetDate = null) {
        ...
        else { const el = document.getElementById('filter-comanda-data'); if (el && el.value) App.comandasState.date = el.value; }
        const currentStatus = App.comandasState.status; const currentDate = App.comandasState.date;
        const tabAbertas = document.getElementById('tab-comandas-abertas'); const tabFechadas = document.getElementById('tab-comandas-fechadas');
        if(tabAbertas) { tabAbertas.classList.toggle('active-tab', currentStatus === 'aberta'); tabAbertas.style.background = currentStatus === 'aberta' ? '#f0f0f0' : 'transparent'; tabAbertas.style.border = currentStatus === 'aberta' ? 'none' : '1px solid transparent'; }
        if(tabFechadas) { tabFechadas.classList.toggle('active-tab', currentStatus === 'fechada'); tabFechadas.style.background = currentStatus === 'fechada' ? '#f0f0f0' : 'transparent'; tabFechadas.style.border = currentStatus === 'fechada' ? 'none' : '1px solid transparent'; }
        ...
        try {
            const { data, error } = await db.from('comandas').select('*, clients(name)').eq('status', currentStatus).gte('created_at', startOfDay).lte('created_at', endOfDay).order('created_at', {ascending: false});
            const container = document.getElementById('comandas-list');
            if (error) throw error;
            if (!data || data.length === 0) {
                const parts = currentDate.split('-'); const brDate = `${parts[2]}/${parts[1]}/${parts[0]}`;
        ...
        } catch (e) { document.getElementById('comandas-list').innerHTML = `<p style="color:var(--muted)">Erro ao carregar: ${e.message}</p>`; }
    },
    async mensagens() {
    ...
    async comissao() {
        ...
        if(dashContainer) {
            ...
            } else { btnClear.classList.add('hidden'); infoDiv.innerHTML = ''; if(defaultSel) defaultSel.style.display = 'block'; }
        }
        let query = db.from('comandas').select('*, clients(name)').order('created_at', {ascending: false});
        if (App.filters.comissoes) {
            let f = App.filters.comissoes; query = query.gte('created_at', f.start + 'T00:00:00Z').lte('created_at', f.end + 'T23:59:59Z').eq('status', 'fechada');
        } else {
            const qFilter = document.getElementById('filter-comissao-quinzena')?.value || U.getCurrentQuinzenaValue();
            const range = U.getQuinzenaDates(qFilter); query = query.gte('created_at', range.start).lte('created_at', range.end).eq('status', 'fechada');
        }
        ...
        data.forEach(c => {
            if(!c.items) return; const clientName = c.clients?.name || 'Cliente';
            const dateTimeStr = U.date(c.created_at); const parts = dateTimeStr.split(/[\s,]+/); const dateStr = parts[0]; const timeStr = parts[1] || '00:00';
            c.items.forEach(i => {
                let showCard = false;
                if (isOwner) { if (profFilterActive) { if (i.prof_id !== App.filters.comissoes.prof_id) return; } showCard = true; }
                else { if (i.prof_id !== App.user.id) return; showCard = true; }
                if(i.commission) {
                    const v = (i.price * i.commission) / 100;
                    if (isOwner) { if(i.prof_id) { if(!rank[i.prof_id]) rank[i.prof_id] = { name: i.prof_name, total: 0 }; rank[i.prof_id].total += v; } }
                    if (showCard) { totalComissao += v; details.push({ date: dateStr, time: timeStr, client: clientName, service: i.name, prof_id: i.prof_id, prof_name: i.prof_name, pct: i.commission, val: v, ticket: c.ticket }); }
                }
            });
        });
        const groupedDetails = {}; details.forEach(d => { if(!groupedDetails[d.date]) groupedDetails[d.date] = []; groupedDetails[d.date].push(d); });
        let htmlCards = '';
        if (details.length === 0) { htmlCards = '<p style="color:var(--muted)">Nenhum registro encontrado para este período.</p>'; }
        else {
            for (const [date, items] of Object.entries(groupedDetails)) {
                htmlCards += `<div style="margin-bottom: 25px;"><div style="background: var(--primary-light); color: var(--primary-dark); padding: 8px 15px; border-radius: 8px; margin-bottom: 15px; display:inline-block; font-weight:bold; font-size:0.9rem;"><i class="ph ph-calendar"></i> ${date}</div><div class="data-list">`;
                items.forEach(i => {
                    ...
                    <div><h4 style="font-size:1.1rem; margin-bottom:5px; color:var(--text);">${i.service}</h4>
                    ...
                    <span><i class="ph ph-receipt"></i> Ref: ${i.ticket||'-'} &nbsp;&bull;&nbsp; Taxa: ${i.pct}%</span></div></div>
                    <div class="val" style="color:#2e7d32; font-size:1.4rem">+${U.money(i.val)}</div></div>`;
                });
                htmlCards += `</div></div>`;
            }
        }
        ...
        if(isOwner) {
            ...
            <h3 style="color:white; font-weight:400; opacity:0.9">${titleTotal}</h3><div class="val" style="color:white; font-size:3rem; margin-top:10px">${U.money(totalComissao)}</div></div>`;
            if (profFilterActive) { finalHtml += `<h3 style="margin:20px 0 15px 0">Detalhamento por Dia</h3>${htmlCards}`; }
            else {
                ...
                sortedRank.map((s,i) => {
                    ...
                    const profId = s[0]; const profName = s[1].name; const val = s[1].total;
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center"><h4 style="font-size:1.1rem">${i+1}º ${profName}</h4><i class="ph ph-medal" style="color:${color}; font-size:2rem"></i></div>
                    <div class="val" style="margin-top:15px; font-size:1.8rem">${U.money(val)}</div>
                    <div style="margin-top:10px; font-size:0.8rem; color:var(--primary); font-weight:bold; text-align:right;">Ver Detalhes <i class="ph ph-arrow-right"></i></div></div>`;
                }).join('') + '</div>';
            ...
            <h4 style="color:white; font-weight:400">Minha Comissão Total</h4><div class="val" style="color:white; font-size:3rem; margin-top:10px">${U.money(totalComissao)}</div></div>
            <h3 style="margin:20px 0 15px 0">Meus Detalhamentos por Dia</h3>${htmlCards}`;
        }
        if(dashContainer) dashContainer.innerHTML = finalHtml;
    },
    
    // === ALTERAÇÃO: NOVA RENDERIZAÇÃO DO FLUXO DE CAIXA ===
    async 'resumo-financeiro'() {
        // Renderizar Header de Status
        ...
        const periodDates = document.getElementById('current-period-dates');
        
        if(periodDisplay) {
            periodDisplay.textContent = currentQ.split('-')[2] === 'Q1' ? '1ª Quinzena' : '2ª Quinzena';
        }
        if(periodDates) {
            periodDates.textContent = `${U.date(range.start).slice(0,10)} até ${U.date(range.end).slice(0,10)}`;
        }

        // Botão de Receita Manual
    ...
    async relatorios() {
        ...
        if(relContainer) {
            ...
            // Carregar opções incluindo períodos arquivados
            if(defaultSel && !App.filters.relatorios) {
                let optsHtml = U.generateQuinzenasOptions();
                const { data: archived } = await db.from('financial_periods').select('period_label, start_date, end_date').order('end_date', {ascending: false});
                if(archived && archived.length > 0) {
                    optsHtml += `<optgroup label="📁 Arquivados">`;
                    archived.forEach(p => {
                        if(!optsHtml.includes(`value="${p.period_label}"`)) {
                            const [y, m, q] = p.period_label.split('-');
                            const label = `${q === 'Q1' ? '1ª' : '2ª'} Quinzena (${m}/${y}) - FECHADO`;
                            optsHtml += `<option value="${p.period_label}">${label}</option>`;
                        }
                    });
                    optsHtml += `</optgroup>`;
                }
        ...
        if (App.filters.relatorios) {
            ...
            desp = data || [];
            if (f.tipo === 'Comissões' && f.prof_name) { desp = desp.filter(d => d.description.includes(`Comissão ${f.prof_name}:`)); }
        } else {
            const qFilter = document.getElementById('filter-relatorios').value;
            // Verifica se é um período arquivado
            const { data: archivedPeriod } = await db.from('financial_periods').select('*').eq('period_label', qFilter).maybeSingle();
            if(archivedPeriod) {
                // Se for arquivado, busca na tabela de despesas original seria complexo sem foreign key
                // Para simplificar, mostramos mensagem ou buscamos se ainda existirem dados (caso não tenha sido deletado)
                // Mas como a lógica nova deleta, precisamos avisar ou buscar de outro lugar.
                // Por enquanto, mantemos a busca normal que retornará vazio se já foi fechado e deletado.
                // O ideal futuro é ter uma tabela de 'financial_entries' separada.
                // Como paliativo, vamos assumir que o usuário quer ver o resumo salvo.
                // Mas como o pedido era mover dados, a tabela despesas estará vazia para esse periodo.
                // Vamos deixar a query normal. Se estiver vazio, o usuario sabe que foi arquivado.
                // MELHORIA: Poderiamos salvar o JSON do extrato na financial_periods.
            }
        ...
        let targetList = isReceitaFilter ? desp.filter(d => App.inflowCategories.includes(d.category)) : desp.filter(d => !App.inflowCategories.includes(d.category));
        let htmlDesp = '';
        ...
        window.currentDespesasData = targetList; window.currentRelatorioTipo = isReceitaFilter ? 'Receitas' : 'Despesas';
        let htmlFluxo = '';
...
const Modals = {
    async open(type, param1=null, param2=null, param3=null) {
        ...
        if (type === 'fechar_quinzena_resumo') {
            ...
            html += `
                <div style="text-align:center; margin-bottom:20px;">
                    <div style="width:60px; height:60px; background:var(--primary-light); border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 15px auto;">
                        <i class="ph ph-calendar-check" style="font-size:2rem; color:var(--primary);"></i>
                    </div>
                    <h3 style="margin:0; color:var(--primary-dark)">Resumo Final da Quinzena</h3>
                    <p style="color:var(--muted)">Confira os valores antes de arquivar.</p>
                </div>
                
                <div style="background:#f9f9f9; border-radius:12px; padding:20px; margin-bottom:20px; border:1px solid var(--border);">
                    <div style="display:flex; justify-content:space-between; margin-bottom:10px; padding-bottom:10px; border-bottom:1px dashed #ddd;">
                        <span>Total Entradas:</span> <b style="color:#2e7d32">+${U.money(totalIn)}</b>
                    </div>
                    <div style="display:flex; justify-content:space-between; margin-bottom:10px; padding-bottom:10px; border-bottom:1px dashed #ddd;">
                        <span>Total Saídas:</span> <b style="color:#d32f2f">-${U.money(totalOut)}</b>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:1.3rem; font-weight:bold; color:${lucro >= 0 ? '#2e7d32' : '#d32f2f'}">
                        <span>Lucro Líquido:</span> <span>${U.money(lucro)}</span>
                    </div>
                </div>
                
                <div style="background:#fff3e0; padding:12px; border-radius:8px; margin-bottom:20px; font-size:0.9rem; color:#e65100;">
                    <i class="ph ph-warning"></i> Ao confirmar, você escolherá a data de corte. Todos os lançamentos até essa data serão arquivados permanentemente nesta tela.
                </div>
                
                <button class="btn-primary" style="padding:1.2rem;" onclick="Modals.open('fechar_quinzena_data')">Continuar para Escolher Data</button>
            `;
        }
            ...
            html += `
                <div style="text-align:center; margin-bottom:20px;">
                    <h3 style="margin:0; color:var(--primary-dark)">Definir Data de Fechamento</h3>
                    <p style="color:var(--muted)">Selecione o dia limite desta quinzena.</p>
                </div>
                
                <form onsubmit="Actions.closeQuinzena(event)">
                    <div class="input-group">
                        <label>Data Final da Quinzena</label>
                        <input type="date" id="close-date-input" value="${today}" max="${today}" required style="font-size:1.2rem; padding:1.2rem; text-align:center;">
                        <p style="font-size:0.8rem; color:var(--muted); margin-top:5px; text-align:center;">Tudo após esta data permanecerá na tela atual.</p>
                    </div>
                    
                    <div style="display:flex; gap:10px; margin-top:20px;">
                        <button type="button" class="btn-secondary" style="flex:1; padding:1.2rem;" onclick="Modals.open('fechar_quinzena_resumo')">Voltar</button>
                        <button type="submit" class="btn-primary" style="flex:1; padding:1.2rem; background:#2e7d32;">Confirmar Fechamento</button>
                    </div>
                </form>
            `;
        }
        // === FIM NOVOS MODAIS ===
            ...
            if (isBlocked) {
                let isAdminBlock = a.notes && a.notes.includes('ADMIN_BLOCK'); let endStr = ''; let motivo = a.notes || '';
                if (motivo.includes('BLOQUEIO_ATE:')) { const parts = motivo.split('|'); endStr = parts[0].replace('BLOQUEIO_ATE:', '').trim(); motivo = parts.slice(1).filter(p => !p.includes('ADMIN_BLOCK')).join(' | ').trim(); }
                let removerBtnHtml = '';
                if (isAdminBlock && App.role !== 'owner') { removerBtnHtml = `<p style="color: #d32f2f; text-align:center; font-weight:bold; margin-top:10px;"><i class="ph ph-lock"></i> Bloqueio Administrativo (Apenas gestor pode remover)</p>`; }
                else { removerBtnHtml = `<button class="btn-primary" style="background:#d32f2f; padding:1.2rem; width:100%" onclick="Actions.deleteAppointment('${a.id}')"><i class="ph ph-trash"></i> Remover Bloqueio</button>`; }
                ...
                <p><strong>Motivo:</strong> ${motivo}</p></div>${removerBtnHtml}`;
            } else {
                ...
                <p style="margin-top:5px; border-top:1px dashed #ccc; padding-top:5px;"><strong><i class="ph ph-user-plus"></i> Agendado por:</strong> ${a.created_by_name || 'Desconhecido'}</p></div>
                ...
                <button class="btn-secondary" style="color:#d32f2f; border:1px solid #d32f2f; padding:1.2rem; width:100%" onclick="Actions.cancelAppointment('${a.id}')"><i class="ph ph-x"></i> Cancelar</button></div>`;
            }
        }
            ...
            <button class="btn-secondary" style="padding:1.2rem; font-size:1.1rem; border: 1px solid #d32f2f; color: #d32f2f; background: #ffebee; justify-content:flex-start" onclick="Modals.open('bloquear_agenda', '${param1}', '${param2}', '${param3}')"><i class="ph ph-prohibit" style="font-size:1.5rem; margin-right:10px;"></i> Bloquear Horário</button></div>`;
        }
        else if(type === 'menu_indisponivel') {
            const isMeOrOwner = App.user.id === param1 || App.role === 'owner'; const firstName = param2.split(' ')[0];
            html += `<div style="text-align: center; margin-bottom: 20px;"><h3 style="margin: 0; color: #d32f2f;"><i class="ph ph-calendar-x"></i> Profissional de Folga</h3><p style="color:var(--muted); margin-top:5px;">${firstName} não está escalado(a) para trabalhar neste dia da semana.</p></div>
            <div style="display:flex; flex-direction:column; gap:10px;">
            <button class="btn-primary" style="padding:1.2rem; font-size:1.1rem; justify-content:center; background:#25D366; color:white; border:none" onclick="Actions.consultarDisponibilidade('${param3}', '${firstName}')"><i class="ph ph-whatsapp-logo" style="font-size:1.5rem; margin-right:10px;"></i> Consultar Disponibilidade</button>
            ${isMeOrOwner ? `<button class="btn-secondary" style="padding:1.2rem; font-size:1.1rem; border: 1px solid var(--primary); color: var(--primary); justify-content:center; margin-top:10px;" onclick="Actions.liberarDia('${param1}')"><i class="ph ph-lock-key-open" style="font-size:1.5rem; margin-right:10px;"></i> Liberar Minha Agenda Hoje</button>` : ''}</div>`;
        }
            ...
            <div><span style="font-size:0.7rem; background:var(--bg); padding:3px 8px; border-radius:10px; margin-right:8px; border:1px solid var(--border); font-weight:bold">${i.type==='product'?'PROD':'SERV'}</span>
            <span style="font-size:1.1rem">${i.name}</span><span style="font-size:0.85rem; color:var(--muted); display:block; margin-top:4px;"><i class="ph ph-identification-badge"></i> Profissional: ${i.prof_name || 'Não informado'}</span></div>
            ...
            ${!isFechada ? `<button type="button" onclick="Actions.editComandaItemPrice('${comanda.id}', ${idx})" style="background:none; border:none; color:var(--primary); cursor:pointer; font-size:1.2rem"><i class="ph ph-pencil"></i></button>
            <button type="button" onclick="Actions.removeComandaItem('${comanda.id}', ${idx})" style="background:none; border:none; color:#d32f2f; cursor:pointer; font-size:1.2rem"><i class="ph ph-trash"></i></button>`:''}</div></div>`).join('');
            ...
            html += `<div style="text-align: left; margin-bottom: 20px;"><div style="display:flex; justify-content:space-between; align-items:center;"><h3 style="margin: 0; color: var(--primary-dark);">Ticket: <span style="color:var(--primary)">${comanda.ticket || '-'}</span></h3>${delBtnHtml}</div>
            <p style="color: var(--muted); margin-top: 5px; font-size:1.1rem;">Cliente: <b style="color:var(--text)">${comanda.clients?.name}</b></p></div>
            <div style="background: #fafafa; border: 1px solid var(--border); border-radius: 12px; padding: 15px; margin-bottom: 20px;">
            ...
            <div style="display: flex; justify-content: space-between; border-top: 2px dashed #ccc; padding-top: 15px; margin-top: 15px;"><span style="font-size: 1.2rem; font-weight: bold;">Total Final</span><span style="font-size: 1.8rem; font-weight: bold; color: var(--primary-dark);">${U.money(comanda.total)}</span></div></div>`;
            if(!isFechada) {
                const sOpts = servicos.map(s => { const val = encodeURIComponent(JSON.stringify({id: s.id, name: s.name, price: s.price, cost: s.cost || 0, commission: s.commission, type: 'service'})); return `<li onclick="CustomSelect.select('add-item-sel', '${val}', '${s.name.replace(/'/g, "\\'")} - ${U.money(s.price)}')">${s.name} - ${U.money(s.price)}</li>`; }).join('');
                const pOpts = produtos.map(p => { const val = encodeURIComponent(JSON.stringify({id: p.id, name: p.name, price: p.price, commission: p.commission, type: 'product'})); return `<li onclick="CustomSelect.select('add-item-sel', '${val}', '${p.name.replace(/'/g, "\\'")} (Est. ${p.stock}) - ${U.money(p.price)}')">${p.name} (Est. ${p.stock}) - ${U.money(p.price)}</li>`; }).join('');
                ...
                <div class="input-group" style="margin:0; flex:1; min-width:200px;"><label>Adicionar Item</label>${CustomSelect.render('add-item-sel', '-- Buscar Serviço/Prod --', optionsItemHtml)}</div>
                <div class="input-group" style="margin:0; flex:1; min-width:150px;"><label>Feito por:</label>${CustomSelect.render('add-item-prof', '-- Profissional --', profOpts)}</div>
                <button type="button" class="btn-secondary" style="width:auto; padding:1.2rem;" onclick="Actions.addComandaItem('${comanda.id}')"><i class="ph ph-plus"></i></button></div>
            ...
            html += `<div style="text-align: center; margin-bottom: 20px;"><h3 style="margin: 0; color: var(--primary-dark);">Alterar valor de: ${item.name}</h3><p style="color:var(--muted); margin-top:5px;">Valor atual: <b>${U.money(item.price)}</b></p></div>
            <form onsubmit="Actions.saveComandaItemPrice(event, '${param1}', ${param2})">
            <div class="input-group"><label>Digite o novo valor (R$)</label><input type="number" id="edit-price-val" step="0.01" min="0" value="${item.price}" required style="padding:1.2rem; font-size: 1.2rem;"></div>
            <div style="display:flex; gap:10px; margin-top: 20px;"><button type="button" class="btn-secondary" style="flex:1; padding: 1.2rem;" onclick="Modals.open('edit_comanda', '${param1}')">Cancelar</button><button type="submit" class="btn-primary" style="flex:1; padding: 1.2rem;">Salvar OK</button></div></form>`;
        }
        else if(type === 'agendamento') {
            const [c, s, u] = await Promise.all([ db.from('clients').select('id,name').order('name'), db.from('services').select('id,name,duration').order('name'), db.from('users').select('id,name').neq('username', 'admin.teste').neq('is_deleted', true).eq('active', true).order('name') ]);
            const cliOpts = `<li onclick="CustomSelect.select('fa-cli', 'NEW', '+ CADASTRAR NOVO CLIENTE AQUI')" style="font-weight:bold; color:#2e7d32;">+ CADASTRAR NOVO CLIENTE AQUI</li>` + c.data.map(x=>`<li onclick="CustomSelect.select('fa-cli', '${x.id}', '${x.name.replace(/'/g, "\\'")}')">${x.name}</li>`).join('');
            ...
            const userOpts = u.data.map(x => { if(x.id === param1) { defaultUserName = x.name; defaultUserId = x.id; } return `<li onclick="CustomSelect.select('fa-user', '${x.id}', '${x.name.replace(/'/g, "\\'")}')">${x.name}</li>`; }).join('');
            ...
            <div class="input-group"><label>Cliente</label>${CustomSelect.render('fa-cli', '-- Buscar Cliente --', cliOpts, 'handleNewClient')}</div>
            ...
            <div class="input-group" style="margin:0;"><label>WhatsApp com DDD</label><input type="text" id="fa-new-fone" placeholder="Ex: 86999999999"></div></div>
            <div class="input-group"><label>Serviço</label>${CustomSelect.render('fa-serv', '-- Buscar Serviço --', servOpts)}</div>
            <div class="input-group"><label>Profissional</label>${CustomSelect.render('fa-user', defaultUserName, userOpts, '', defaultUserId)}</div>
            ...
            if (App.role === 'owner') {
                ...
                profSelectHtml = `<div class="input-group" style="background:#fff3e0; border: 1px solid #ffb74d; padding:15px; border-radius:12px;">
                <label style="color:#e65100; font-weight:bold"><i class="ph ph-identification-badge"></i> Qual profissional bloquear?</label>
                <select id="fb-user" required style="border-color:#ffb74d"><option value="${App.user.id}" ${App.user.id===param1?'selected':''}>Bloquear a Mim Mesmo(a)</option>
                ${users.filter(u => u.id !== App.user.id).map(x=>`<option value="${x.id}" ${x.id===param1?'selected':''}>${x.name}</option>`).join('')}</select></div>`;
            }
            html += `<h3 style="text-align:center; margin-bottom:20px; color:#d32f2f">Bloquear Horário</h3><form onsubmit="Actions.blockAppointment(event)">${profSelectHtml}
            ...
            const cliOptsCom = `<li onclick="CustomSelect.select('fcom-cli', 'NEW', '+ CADASTRAR NOVO CLIENTE AQUI')" style="font-weight:bold; color:#2e7d32;">+ CADASTRAR NOVO CLIENTE AQUI</li>` + c.map(x=>`<li onclick="CustomSelect.select('fcom-cli', '${x.id}', '${x.name.replace(/'/g, "\\'")}')">${x.name}</li>`).join('');
            html += `<h3>Gerar Novo Ticket</h3><form onsubmit="Actions.createComanda(event)">
            <div class="input-group"><label>Cliente</label>${CustomSelect.render('fcom-cli', '-- Buscar Cliente --', cliOptsCom, 'handleNewClientComanda')}</div>
            ...
            <div class="input-group" style="margin:0;"><label>Data de Nascimento (Opcional)</label><input type="date" id="fcom-new-nasc"></div></div>
            ...
            <p style="color:var(--muted); font-size:0.9rem; margin-bottom:5px">Ref: ${param3 || 'Pendente'}</p><b style="font-size:2rem; color:#2e7d32">${U.money(param2)}</b></div>
            ...
            <div class="input-group" style="margin:0"><label><i class="ph ph-credit-card"></i> Cartão Débito</label><input type="number" id="pay-debito" step="0.01" min="0" placeholder="R$ 0,00"></div></div>
            <button type="submit" id="btn-pay" class="btn-primary" style="background:#2e7d32; padding:1.2rem">Confirmar Pagamentos</button></form>`;
        }
            ...
            html += `<h3>Nova Observação</h3><form id="form-nova-obs" data-client-id="${param1}" onsubmit="Actions.saveObservacao(event)">
            <div class="input-group"><label>Técnica Aplicada (Não obrigatório)</label><textarea id="fo-tecnica" rows="2" placeholder="Qual técnica foi utilizada?"></textarea></div>
            <div class="input-group"><label>Observação (Não obrigatório)</label><textarea id="fo-obs" rows="3" placeholder="Detalhes, fórmulas, restrições..."></textarea></div>
            <button type="submit" class="btn-primary" style="padding:1.2rem">Salvar Registro</button></form>`;
        }
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;"><div class="input-group" style="flex:1; min-width:150px;"><label>Telefone/WhatsApp</label><input type="text" id="ff-fone" value="${f.phone || ''}" placeholder="Ex: 86999999999"></div>
            <div class="input-group" style="flex:1; min-width:150px;"><label>Data de Nascimento</label><input type="date" id="ff-nasc" value="${f.birth_date || ''}"></div></div>
            ...
            <label style="cursor:pointer; background:#fff; padding:6px 12px; border-radius:6px; border:1px solid #ccc; font-size:0.9rem"><input type="checkbox" id="chk-sab" ${sched["6"]?.active !== false ? 'checked' : ''}> Sáb</label></div></div>
            ...
            html += `<h3>Filtro Avançado (Relatórios)</h3><form onsubmit="Actions.applyFilterRelatorios(event)">
            <div style="display:flex; gap:10px;"><div class="input-group"><label>Data Inicial</label><input type="date" id="frel-start" required></div><div class="input-group"><label>Data Final</label><input type="date" id="frel-end" required></div></div>
            <div class="input-group"><label>Filtrar Qual Movimentação?</label><select id="frel-tipo" required onchange="document.getElementById('frel-prof-div').style.display = this.value === 'Comissões' ? 'block' : 'none'">
            <option value="Todos">Todas (Custos, Comissões e Entradas)</option><option value="Receitas">Apenas Entradas (Receitas)</option><option value="Custos Fixos">Apenas Custos Fixos</option><option value="Comissões">Apenas Comissões</option></select></div>
            <div class="input-group" id="frel-prof-div" style="display:none;"><label>De Qual Colaborador?</label><select id="frel-prof"><option value="">-- Todos os Colaboradores --</option>${uOpts}</select></div>
            <button type="submit" class="btn-primary" style="padding:1.2rem">Pesquisar Período</button></form>`;
        }
        else if (type === 'filtro_comissoes') {
            const { data: users } = await db.from('users').select('id,name').neq('username', 'admin.teste').neq('is_deleted', true).eq('active', true).order('name');
            const uOpts = users.map(u => `<option value="${u.id}|${u.name}">${u.name}</option>`).join('');
            html += `<h3>Filtro de Comissões</h3><form onsubmit="Actions.applyFilterComissoes(event)">
            <div style="display:flex; gap:10px;"><div class="input-group"><label>Data Inicial</label><input type="date" id="fcom-start" required></div><div class="input-group"><label>Data Final</label><input type="date" id="fcom-end" required></div></div>
            ${App.role === 'owner' ? `<div class="input-group"><label>De Qual Colaborador?</label><select id="fcom-prof"><option value="">-- Todos os Colaboradores --</option>${uOpts}</select></div>` : ''}
            <button type="submit" class="btn-primary" style="padding:1.2rem">Pesquisar Período</button></form>`;
        }
        else if (type === 'nova_receita') {
            html += `<h3>Injetar Receita (Entrada)</h3><form onsubmit="Actions.createReceita(event)">
            ...
            <div class="input-group"><label>Forma de Entrada</label><select id="fr-cat" required style="padding:1.2rem"><option value="Pix">Pix</option><option value="Dinheiro">Dinheiro</option><option value="Cartão Crédito">Cartão de Crédito</option><option value="Cartão Débito">Cartão de Débito</option></select></div>
            ...
            <button type="submit" class="btn-primary" style="padding:1.2rem; background:#2e7d32">Confirmar Entrada</button></form>`;
        }
...
const Actions = {
    ...
    async closeQuinzena(e) {
        ...
        try {
            const endDateStr = document.getElementById('close-date-input').value;
            if(!endDateStr) throw new Error('Data inválida.');
            ...
            // 1. Buscar tudo que será arquivado
            ...
            if(toArchive && toArchive.length > 0) {
                toArchive.forEach(d => {
                    ...
                    if(App.inflowCategories.includes(d.category)) totalIn += val;
                    else totalOut += val;
                });
            } else {
                UI.confirm('Não há movimentações neste período. Deseja mesmo fechar e zerar?', async () => {
                    await this.finalizeClose(currentQVal, range.start, endDateFull, 0, 0, 0);
                });
            ...
            // 3. Deletar da tabela ativa (despesas)
            if(toArchive && toArchive.length > 0) {
                const idsToDelete = toArchive.map(d => d.id);
                const { error: deleteErr } = await db.from('despesas').delete().in('id', idsToDelete);
            ...
            UI.toast(`Erro ao fechar: ${err.message}`, 'error');
            btn.disabled = false; btn.innerHTML = originalText;
        }
    },
    
    async finalizeClose(label, start, end, inV, outV, netV) {
        await db.from('financial_periods').insert({
            period_label: label, start_date: start, end_date: end,
            total_in: inV, total_out: outV, net_profit: netV, closed_by: App.user.id
        });
        Modals.close();
        UI.toast('Período encerrado!');
        Render['resumo-financeiro']();
    },
    // === FIM NOVA AÇÃO ===
    ...
    changeComandaDate(dir) {
        ...
        let d = new Date(dateInput.value + 'T12:00:00'); d.setDate(d.getDate() + dir);
        const newDate = U.iso(d); dateInput.value = newDate; Render.comandas(null, newDate);
    },
    consultarDisponibilidade(phone, name) {
        ...
        window.open(`https://wa.me/55${phone.replace(/\D/g, '')}?text=${encodeURIComponent(msg)}`, '_blank'); Modals.close();
    },
    async liberarDia(userId) {
        UI.confirm('Deseja liberar a agenda deste profissional para o dia de hoje?', async () => {
            ...
            if(user) {
                ...
                if(sched[String(currentDayOfWeek)]) { sched[String(currentDayOfWeek)].active = true; }
                await db.from('users').update({ schedule: sched }).eq('id', userId);
                Modals.close(); UI.toast('Agenda liberada com sucesso!'); Render.agendaDay();
            }
        });
    },
    ...
    filterComissaoByProf(profId, profName) {
        let s, e;
        if(App.filters.comissoes) { s = App.filters.comissoes.start; e = App.filters.comissoes.end; }
        else { const qFilter = document.getElementById('filter-comissao-quinzena')?.value || U.getCurrentQuinzenaValue(); const range = U.getQuinzenaDates(qFilter); s = range.start.slice(0,10); e = range.end.slice(0,10); }
        App.filters.comissoes = { start: s, end: e, prof_id: profId, prof_name: profName }; Render.comissao();
    },
    async createReceita(e) {
        e.preventDefault(); const valorReceita = parseFloat(document.getElementById('fr-val').value) || 0;
        await db.from('despesas').insert({ description: "ENTRADA MANUAL: " + document.getElementById('fr-desc').value, amount: valorReceita, category: document.getElementById('fr-cat').value, date: new Date().toISOString() });
        Modals.close(); UI.toast('Valor positivo injetado com sucesso no Fluxo de Caixa!'); Render['resumo-financeiro'](); if (App.view === 'relatorios') { Render.relatorios(); }
    },
    ...
    applyTemplate(templateId) {
        const box = document.getElementById('wpp-msg'); if(!templateId) { box.value = ''; return; }
        db.from('message_templates').select('content').eq('id', templateId).single().then(({ data }) => { if(!data) return; box.value = U.fillTemplate(data.content, window.currentWppVars || {}); });
    },
    ...
    async saveObservacao(e) {
        e.preventDefault(); const btn = e.target.querySelector('button[type="submit"]'); const originalText = btn.innerHTML; btn.innerHTML = 'Salvando...'; btn.disabled = true;
        try {
            const idCliente = e.target.getAttribute('data-client-id');
            if (!idCliente || idCliente === 'undefined' || idCliente === 'null') { throw new Error('Erro Crítico: Cliente não identificado. Volte e tente novamente.'); }
            const tecnica = document.getElementById('fo-tecnica').value.trim(); const obs = document.getElementById('fo-obs').value.trim();
            if (!tecnica && !obs) { throw new Error('Preencha ao menos a técnica ou a observação antes de salvar.'); }
            const { error } = await db.from('anamnesis').insert({ client_id: idCliente, user_id: App.user.id, history: tecnica, notes: obs });
            if (error) throw new Error(error.message);
            Modals.close(); UI.toast('Observação registrada com sucesso!'); await this.loadObservacoes(idCliente);
        } catch (err) { UI.toast(err.message, 'error'); } finally { if (btn) { btn.innerHTML = originalText; btn.disabled = false; } }
    },
    async loadObservacoes(id) {
        const div = document.getElementById('observacao-history-list');
        div.innerHTML = `<div style="text-align:center; padding:3rem 1rem;"><i class="ph ph-spinner ph-spin" style="font-size: 2.5rem; color: var(--primary);"></i><p style="color:var(--muted); margin-top:10px;">Carregando histórico do cliente...</p></div>`;
        const { data, error } = await db.from('anamnesis').select('*, users!user_id(name)').eq('client_id', id).order('created_at', {ascending: false});
        if (error) { div.innerHTML = `<p style="color:#d32f2f; text-align:center; padding:2rem;">Erro ao carregar: ${error.message}</p>`; return; }
        if(!data || !data.length) {
            div.innerHTML = `<div style="text-align:center; padding: 3rem 1rem; border: 2px dashed #eee; border-radius: 12px; margin-top: 20px;"><i class="ph ph-file-text" style="font-size: 3rem; color: var(--muted); margin-bottom: 10px;"></i><p style="color:var(--muted); font-size: 1.1rem;">Nenhum histórico registrado para este cliente.</p><p style="color:var(--muted); font-size: 0.9rem; margin-top: 5px;">Clique em "Nova Observação" para iniciar o prontuário.</p></div>`; return;
        }
        div.innerHTML = `<div style="display: flex; flex-direction: column; gap: 15px; margin-top: 15px;">` + data.map((d, index) => {
            const dataFormatada = U.date(d.created_at); const isLatest = index === 0;
            return `<div class="card" style="border-left: 4px solid ${isLatest ? 'var(--primary)' : '#b0bec5'}; background: ${isLatest ? '#fffafb' : '#ffffff'}; box-shadow: 0 4px 10px rgba(0,0,0,0.03); position:relative;">
            ...
            <div><span style="color: ${isLatest ? 'var(--primary-dark)' : '#546e7a'}; font-size: 0.85rem; font-weight: bold; background: ${isLatest ? 'var(--primary-light)' : '#eceff1'}; padding: 4px 10px; border-radius: 20px;"><i class="ph ph-calendar-check"></i> ${dataFormatada}</span></div>
            <div style="text-align: right;"><span style="font-size: 0.75rem; color: var(--muted);">Registrado por:</span><p style="font-weight: bold; font-size: 0.9rem; color: var(--text); margin-top: 2px;"><i class="ph ph-user-circle"></i> ${d.users?.name || 'Sistema'}</p></div></div>
            <div style="display: flex; flex-direction: column; gap: 10px;">
            ${d.history ? `<div style="background: #f8f9fa; padding: 12px; border-radius: 8px; border-left: 3px solid #6c757d;"><span style="font-size: 0.75rem; color: #6c757d; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 5px;"><i class="ph ph-flask"></i> Técnica Aplicada</span><p style="color: #333; line-height: 1.5; font-size: 0.95rem; margin:0;">${d.history.replace(/\n/g, '<br>')}</p></div>` : ''}
            ${d.notes ? `<div style="background: #fff3e0; padding: 12px; border-radius: 8px; border-left: 3px solid #ff9800;"><span style="font-size: 0.75rem; color: #e65100; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 5px;"><i class="ph ph-warning-circle"></i> Observações / Alertas</span><p style="color: #333; line-height: 1.5; font-size: 0.95rem; margin:0;">${d.notes.replace(/\n/g, '<br>')}</p></div>` : ''}
            </div></div>`;
        }).join('') + `</div>`;
    },
    async saveFuncionario(e, id) {
        e.preventDefault(); const nomeDisplay = document.getElementById('ff-nome').value.trim(); const palavras = nomeDisplay.toLowerCase().split(/\s+/);
        const username = `${palavras[0]}${palavras.length>1?'.'+palavras[palavras.length-1]:''}`.replace(/[^a-z0-9.]/g, '');
        const role = document.getElementById('ff-role').value; const phone = document.getElementById('ff-fone').value; const birthDate = document.getElementById('ff-nasc').value || null;
        const schedule = { "0": { active: document.getElementById('chk-dom').checked }, "1": { active: document.getElementById('chk-seg').checked }, "2": { active: document.getElementById('chk-ter').checked }, "3": { active: document.getElementById('chk-qua').checked }, "4": { active: document.getElementById('chk-qui').checked }, "5": { active: document.getElementById('chk-sex').checked }, "6": { active: document.getElementById('chk-sab').checked } };
        const payload = { name: nomeDisplay, role: role, phone: phone, birth_date: birthDate, schedule: schedule };
        if(id) { payload.active = document.getElementById('ff-ativo').checked; await db.from('users').update(payload).eq('id', id); Modals.close(); UI.toast('Atualizado com sucesso!'); Render.funcionarios(); }
        else {
            payload.username = username; payload.password = '123456'; payload.first_login = true; payload.active = true; payload.is_deleted = false;
            await db.from('users').insert(payload); UI.confirm(`Usuário Criado: ${username}\nSenha Temp: 123456`, () => { Modals.close(); Render.funcionarios(); });
        }
    },
    ...
    async createAppointment(e) {
        ...
        try {
            ...
            if(clientId === 'NEW') {
                const newNome = document.getElementById('fa-new-nome').value.trim(); const newFone = document.getElementById('fa-new-fone').value.trim();
                ...
                if(cliErr || !newCli) throw new Error('Erro ao salvar o cliente novo no banco de dados.'); clientId = newCli.id; Render.clientes();
            }
            const servVal = document.getElementById('fa-serv').value; if(!servVal) throw new Error('Selecione o serviço!');
            const [service_id, durStr] = servVal.split('|'); const dur = parseInt(durStr || 60); const encaixe = document.getElementById('fa-encaixe').checked;
            ...
            let conflitoNormal = false; let conflitoAdmin = false;
            if(over) {
                const sM = (time.split(':')[0]*60)+Number(time.split(':')[1]); const eM = sM + dur;
                over.forEach(a => {
                    const [ash, asm] = (a.time||'00:00').split(':').map(Number); let aEM;
                    if(a.status === 'bloqueado' && a.notes?.includes('BLOQUEIO_ATE:')) { aEM = (a.notes.split('|')[0].replace('BLOQUEIO_ATE:','').trim().split(':')[0]*60)+Number(a.notes.split('|')[0].replace('BLOQUEIO_ATE:','').trim().split(':')[1]); }
                    else { aEM = (ash*60+asm) + (a.services?.duration||60); }
                    const aSM = ash*60+asm; if(sM < aEM && eM > aSM) { if (a.status === 'bloqueado' && a.notes?.includes('ADMIN_BLOCK')) { conflitoAdmin = true; } else { conflitoNormal = true; } }
                });
            }
            if (conflitoAdmin && App.role !== 'owner') { throw new Error('Horário bloqueado pelo Gestor. O Encaixe não é permitido neste caso.'); }
            if ((conflitoNormal || conflitoAdmin) && !encaixe) { throw new Error('Horário ocupado! Marque a opção "Forçar Encaixe" se for estritamente necessário.'); }
            const { error: insertErr } = await db.from('appointments').insert({ client_id: clientId, service_id: service_id, user_id: user_id, date: date, time: time, is_encaixe: encaixe, status: 'agendado', created_by_name: App.user.name });
            if (insertErr) throw new Error(`Falha ao salvar no banco: ${insertErr.message}`);
            Modals.close(); UI.toast('Horário salvo com sucesso!'); Render.agendaDay();
        } catch(err) { UI.toast(err.message, 'error'); } finally { if (btn) { btn.innerHTML = originalText; btn.disabled = false; } }
    },
    async blockAppointment(e) {
        ...
        const userTargetObj = document.getElementById('fb-user'); const targetUserId = userTargetObj ? userTargetObj.value : App.user.id; const isAdminBlock = App.role === 'owner' ? ' | ADMIN_BLOCK' : '';
        if (start >= end) return UI.toast('Término deve ser maior que início.', 'error');
        await db.from('appointments').insert({ user_id: targetUserId, date: date, time: start, status: 'bloqueado', notes: `BLOQUEIO_ATE:${end} | ${motivo}${isAdminBlock}` });
        Modals.close(); UI.toast('Horário bloqueado com sucesso!'); Render.agendaDay();
    },
    ...
    async createComanda(e) {
        e.preventDefault(); const btn = document.getElementById('btn-gera-comanda'); const originalText = btn.innerHTML; btn.disabled = true; btn.innerHTML = "Gerando...";
        try {
            let clientId = document.getElementById('fcom-cli').value;
            if(clientId === 'NEW') {
                const newNome = document.getElementById('fcom-new-nome').value.trim(); const newFone = document.getElementById('fcom-new-fone').value.trim(); const newNasc = document.getElementById('fcom-new-nasc').value;
                if(!newNome) throw new Error('Preencha o nome do novo cliente.');
            ...
            const { data } = await db.from('comandas').select('ticket'); let maxNum = 0; (data || []).forEach(c => { if (c.ticket) { const n = parseInt(c.ticket.split('-')[1], 10); if (n > maxNum) maxNum = n; } });
        ...
        } catch(err) { UI.toast(err.message, 'error'); } finally { if(btn) { btn.disabled = false; btn.innerHTML = originalText; } }
    },
    async addComandaItem(id) {
        const val = document.getElementById('add-item-sel').value; const profVal = document.getElementById('add-item-prof').value;
        if(!val) return UI.toast('Selecione um serviço ou produto.', 'error'); if(!profVal) return UI.toast('Selecione o profissional que realizou o serviço!', 'error');
        const item = JSON.parse(val); const [prof_id, prof_name] = profVal.split('|'); item.prof_id = prof_id; item.prof_name = prof_name;
        if(item.type === 'product') { const { data: p } = await db.from('products').select('stock').eq('id', item.id).single(); if(p.stock <= 0) return UI.toast('Sem estoque.', 'error'); await db.from('products').update({stock: p.stock - 1}).eq('id', item.id); }
        const { data: c } = await db.from('comandas').select('items, total').eq('id', id).single(); const items = c.items || []; items.push(item);
        await db.from('comandas').update({ items, total: c.total + item.price }).eq('id', id); Modals.close(); setTimeout(() => Modals.open('edit_comanda', id), 100);
    },
    async removeComandaItem(id, idx) {
        const { data: c } = await db.from('comandas').select('items, total').eq('id', id).single(); const items = c.items || []; const item = items[idx];
        if(item.type === 'product') { const { data: p } = await db.from('products').select('stock').eq('id', item.id).single(); if(p) await db.from('products').update({stock: p.stock + 1}).eq('id', item.id); }
        items.splice(idx, 1); await db.from('comandas').update({ items, total: Math.max(0, c.total - item.price) }).eq('id', id); Modals.close(); setTimeout(() => Modals.open('edit_comanda', id), 100);
    },
    ...
    async saveComandaItemPrice(e, id, idx) {
        e.preventDefault(); const novoValorStr = document.getElementById('edit-price-val').value; const novoValor = parseFloat(novoValorStr);
        if (isNaN(novoValor) || novoValor < 0) return UI.toast('Valor inválido.', 'error');
        const { data: c } = await db.from('comandas').select('items, total').eq('id', id).single(); const items = c.items || []; const item = items[idx]; const diferenca = novoValor - item.price; item.price = novoValor;
        await db.from('comandas').update({ items, total: Math.max(0, c.total + diferenca) }).eq('id', id); UI.toast('Valor modificado com sucesso!'); Modals.close(); setTimeout(() => Modals.open('edit_comanda', id), 100);
    },
    async closeComanda(comandaId, clientId, total, ticketNum) {
        UI.confirm('Deseja fechar esta comanda? Ela gerará os custos e separará as comissões por profissional automaticamente, enviando para cobrança.', async () => {
            const btn = document.getElementById('btn-fechar-com'); if(btn) { btn.disabled = true; btn.innerHTML = "Processando..."; }
            const { data: comanda } = await db.from('comandas').select('items, clients(name)').eq('id', comandaId).single(); const clientName = comanda.clients?.name || 'Cliente';
            let totalCustoFixo = 0; let commissionsByProf = {};
            if(comanda.items) {
                comanda.items.forEach(item => {
                    let com = 0; if (item.type === 'service' && item.commission) com = (item.price * item.commission) / 100; if (item.type === 'product' && item.commission) com = (item.price * item.commission) / 100;
                    if (item.type === 'service' && item.cost) { totalCustoFixo += (item.price * item.cost) / 100; }
                    if (com > 0) { let pName = item.prof_name || 'Profissional Desconhecido'; commissionsByProf[pName] = (commissionsByProf[pName] || 0) + com; }
                });
            }
            const dtISO = new Date().toISOString(); await db.from('comandas').update({ status: 'fechada', created_at: dtISO }).eq('id', comandaId);
            if(total > 0) {
            ...
            for (const [pName, val] of Object.entries(commissionsByProf)) { await db.from('despesas').insert({ description: `Comissão ${pName}: Comanda ${ticketNum} | Cliente: ${clientName}`, amount: val, category: 'Comissões', date: dtISO }); }
    ...
    async reopenComanda(id) {
        UI.confirm('ALERTA: Isso exclui os custos automáticos associados. Continuar?', async () => {
            ...
            if(c?.ticket) {
                ...
                if(d) { let rem = d.comanda_ticket.split(', ').map(t=>t.trim()).filter(t => t !== c.ticket).join(', '); if(rem === '') await db.from('debts').delete().eq('id', d.id); else await db.from('debts').update({ total_amount: Math.max(0, d.total_amount - c.total), remaining_amount: Math.max(0, d.remaining_amount - c.total), comanda_ticket: rem }).eq('id', d.id); }
            }
            await db.from('comandas').update({ status: 'aberta' }).eq('id', id); Modals.close(); UI.toast('Reaberta!'); Render.comandas();
        });
    },
    async deleteComanda(id) { UI.confirm('Tem certeza que deseja deletar permanentemente esta comanda aberta?', async () => { try { await db.from('comandas').delete().eq('id', id); Modals.close(); UI.toast('Comanda deletada com sucesso!'); Render.comandas(); } catch (err) { UI.toast('Erro ao deletar comanda.', 'error'); } }); },
    async saveService(e, id) {
        e.preventDefault(); const btn = e.target.querySelector('button[type="submit"]'); const originalText = btn.innerHTML; btn.innerHTML = 'Salvando...'; btn.disabled = true;
        try {
            const aux = document.getElementById('fs-aux').checked;
            const payload = { name: document.getElementById('fs-nome').value.trim(), price: parseFloat(document.getElementById('fs-valor').value.replace(',', '.')) || 0, cost: parseFloat(document.getElementById('fs-custo').value) || 0, commission: parseFloat(document.getElementById('fs-com').value) || 0, duration: parseInt(document.getElementById('fs-duracao').value) || 60, has_assistant: aux, assistant_commission: aux ? (parseFloat(document.getElementById('fs-auxcom').value) || 0) : 0 };
            let response; if(id) { response = await db.from('services').update(payload).eq('id', id); } else { response = await db.from('services').insert([payload]); }
            if (response.error) throw response.error; Modals.close(); UI.toast('Serviço salvo com sucesso!'); Render.servicos();
        } catch (error) { console.error("Erro no Supabase:", error); UI.toast('Erro ao salvar: ' + error.message, 'error'); } finally { if(btn) { btn.innerHTML = originalText; btn.disabled = false; } }
    },
    async deleteService(id) { UI.confirm('Deseja realmente excluir este serviço?', async () => { try { const { error } = await db.from('services').delete().eq('id', id); if (error) throw error; UI.toast('Serviço excluído!'); Render.servicos(); } catch (error) { console.error("Erro ao excluir:", error); UI.toast('Erro ao excluir: ' + error.message, 'error'); } }); },
    ...
    async debitDebt(e, id, max, refTicket) {
        e.preventDefault(); const btn = document.getElementById('btn-pay'); btn.disabled = true; btn.innerHTML = "Processando...";
        const pix = parseFloat(document.getElementById('pay-pix').value || 0); const din = parseFloat(document.getElementById('pay-dinheiro').value || 0); const cre = parseFloat(document.getElementById('pay-credito').value || 0); const deb = parseFloat(document.getElementById('pay-debito').value || 0);
        ...
        const dtISO = new Date().toISOString(); const { data: currDebt } = await db.from('debts').select('payment_details, comanda_ticket, clients(name)').eq('id', id).single();
        const clientName = currDebt.clients?.name || 'Não informado'; const tkts = currDebt.comanda_ticket || refTicket; const descText = `Pgto Comanda(s): ${tkts} | Cliente: ${clientName}`;
        ...
        let pd = currDebt.payment_details || {}; pd.pix = (pd.pix||0) + pix; pd.dinheiro = (pd.dinheiro||0) + din; pd.credito = (pd.credito||0) + cre; pd.debito = (pd.debito||0) + deb;
        const newRem = Math.max(0, max - totalPaid); await db.from('debts').update({ remaining_amount: newRem, payment_details: pd }).eq('id', id);
        Modals.close(); UI.toast('Pagamento lançado no Fluxo!'); Render.cobrancas(newRem === 0 ? 'pagos' : 'pendentes');
    },
    async discountDebt(e, id, max) {
        e.preventDefault(); const perc = parseFloat(document.getElementById('f-val').value); const { data: currDebt } = await db.from('debts').select('payment_details').eq('id', id).single();
        let pd = currDebt.payment_details || {}; pd.desconto = perc; await db.from('debts').update({ remaining_amount: Math.max(0, max - (max * perc / 100)), payment_details: pd }).eq('id', id); Modals.close(); Render.cobrancas('pendentes');
    },
    ...
    previewAndSaveAvatar(event) {
        ...
        reader.onload = async (e) => {
            const base64 = e.target.result; document.getElementById('perfil-foto-preview').innerHTML = ''; document.getElementById('perfil-foto-preview').style.backgroundImage = `url(${base64})`; document.getElementById('perfil-foto-preview').style.backgroundSize = 'cover'; document.getElementById('perfil-foto-preview').style.backgroundPosition = 'center'; document.getElementById('perfil-foto-preview').style.color = 'transparent';
            try {
                const {data: existing} = await db.from('user_avatars').select('id').eq('user_id', App.user.id).maybeSingle();
                if(existing) { await db.from('user_avatars').update({avatar_base64: base64}).eq('id', existing.id); } else { await db.from('user_avatars').insert({user_id: App.user.id, avatar_base64: base64}); }
                App.avatars[App.user.id] = base64; Auth.updateHeaderAvatar(); Render.agendaDay(); UI.toast('Sua foto de perfil foi atualizada com sucesso!');
            } catch(err) { UI.toast('Erro ao salvar no banco. A tabela user_avatars já foi criada?', 'error'); }
        }; reader.readAsDataURL(file);
    },
    async deleteAvatar() {
        if(!App.avatars[App.user.id]) return UI.toast('Você já não possui foto.', 'warning');
        UI.confirm('Remover sua foto de perfil?', async () => { try { await db.from('user_avatars').delete().eq('user_id', App.user.id); delete App.avatars[App.user.id]; Auth.updateHeaderAvatar(); Nav.showView('perfil'); Render.agendaDay(); UI.toast('Foto removida!'); } catch(e) { UI.toast('Erro ao remover.', 'error'); } });
    }
};
...
const initCSS = () => {
    ...
    const style = document.createElement('style'); style.id = 'aqc-custom-styles';
    style.innerHTML = `
